...
class Reportes extends CI_Controller {
    ...
    function getActividades() {
        ...
        $rows = $this->reportes_model->getActividades($login);
    ...
    function cambiaColor() {
        ...
        $sepudo = $this->reportes_model->cambiacolor($id, $color);
        if ($sepudo)
            echo 'ok'; else
    ...
    function eliminaActividad() {
        $this->load->model("reportes_model");
        $id = $this->input->Post("id");
        $sepudo = $this->reportes_model->elimina_actividad($id);
        if ($sepudo)
            echo 'ok'; else
            echo "Se produjo un error al eliminar la actividad.";
    }
    ...
    function desasignaActividad() {
        $this->load->model("reportes_model");
        $id = $this->input->Post("id");
        $sepudo = $this->reportes_model->desasigna_actividad($id);
        if ($sepudo)
            echo 'ok'; else
            echo "Se produjo un error al eliminar la actividad.";
    }
    ...
    function agregaActividad() {
        ...
        $sepudo = $this->reportes_model->agrega_actividad($nombre, $nombre_corto, $tipo_act, $color, $f1, $f2);
        if ($sepudo)
            echo 'ok'; else
    ...
    function asignaActividad() {
        ...
        $sepudo = $this->reportes_model->asigna_actividad($id_act, $catedratico, $bloque, $seccion);
        if ($sepudo)
            echo 'ok'; else
            echo "Se produjo un error al agregar la Actividad.";
    }
    ...
    function modificaActividad() {
        ...
        $sepudo = $this->reportes_model->modifica_actividad($id, $nombre, $nombre_corto, $color);
        if ($sepudo)
            echo 'ok'; else
    ...
    function actualizaStatusActividades() {
        ...
        $sepudo = $this->reportes_model->actualiza_st_actividades($login, $st);
        if ($sepudo)
            echo 'ok'; else
    ...
    function getActividad() {
        $this->load->model("reportes_model");
        $id = $this->input->Post("id");
        $rows = $this->reportes_model->getactividad($id);
        foreach ($rows->result() as $row) {
    ...
    function getCatedraticosActividad() {
        ...
        $rows = $this->reportes_model->getcatedraticosactividad($id);
        $i = 0;
        foreach ($rows->result() as $row) {
