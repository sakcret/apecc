...
<div id="f_agregar_usuario" title="Agregar un Usuario" style="display:none;">
    ...
    <form method="POST" action="" id="form_agrega_usuario">
        <fieldset>
            ...
            <input name="foto" id="foto" type="checkbox" value="si" /-->		
        </fieldset>
    </form>
</div>
...
<div id="f_modificar_usuario" title="Modificar Usuario" style="display:none;">
    ...
    <form method="POST" action="" id="form_modifica_usuario">
        <fieldset>
            ...
            <label for="m_tipo_u">Tipo de usuario*:</label>
            <div class="ui-corner-all">
                <select name="m_tipo_u" class="ui-widget-content combobox-ui" id="m_tipo_u">
                    <option value="0">Selecciona tipo de usuario</option>
                    <?php
                    if (isset($tipos_u_rows) && ($tipos_u_rows != false)) {
                        foreach ($tipos_u_rows->result() as $row) {
...
<div align="center" id="ver_campos" class="hide row ui-widget-content ui-corner-all boxshadowround" style=" width: 70%;">
    <center>
        ...
        <div id="vo_campos" style=" margin-bottom: 10px; margin-top: 10px;">
            ...
            <input type="checkbox" onclick="verOcultarColDT(4,dt_usuarios);" checked="checked" name="vo_fecha_c" id="vo_fecha_c" ><label for="vo_fecha_c">Fecha de creaci&oacute;n</label>
            <input type="checkbox" onclick="verOcultarColDT(5,dt_usuarios);" checked="checked" name="vo_fecha_e" id="vo_fecha_e" ><label for="vo_fecha_e">Fecha de expiraci&oacute;n</label>
...
<script type="text/javascript" charset="utf-8">
    ...
    function modifica_usuario(id){ 
        ...
        ncred = $( "#m_ncredencial" ),
        //foto = $( "#m_foto" ),
        allFields = $( [] ).add( login ).add( matricula).add(nombre)
    ...
    $(document).ready(function() { 
        ...
        /*obtener los datos del registro seleccionado y guardarlos en una variable global*/
        $("#dtusuarios tbody tr").live("click", function(){
            var anSelected = fnGetSelected(dt_usuarios);
            row_select=dt_usuarios.fnGetData(anSelected[0]);
        }); 
        ...
        $( "#f_agregar_usuario" ).dialog({
            ...
            buttons: {
                "Agregar Usuario": function() {                    
                    ...
                    bValid = bValid && validaCampoExpReg(amaterno,/^[A-Za-z\s áéíóúñÁÉÍÓÚÑ]*$/,"Por favor ingresa un 'Apellido Materno' válido.",tips );
                    if($("#tipo_u").val()==0){
                        $("#tipo_u").addClass( "ui-state-error" );
                        updateTips("Debe selecionar un tipo de usuario",tips);
                        bValid = bValid && false;
                    }
                    
                    
                    if(esCampoVacio(matricula)==false){
                        bValid = bValid && validaCampoExpReg(matricula,/^\S\d{8}$/,"Matricula inválida... El formato de la 'Matricula debe ser: una letra S seguida de 8 números \nEjemplo: S07010150.",tips );
                    } else{}
                    if ( bValid ) {
                        login.val(calcLogin(nombre,apaterno,amaterno));   
                        var datos = $( "#form_agrega_usuario" ).serialize();
